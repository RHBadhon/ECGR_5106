...
class TransProModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        # #check the real_A projection map
        # print("the projeciton of OCT is ")
        # imslice3d = self.real_A_proj_test.squeeze().cpu().numpy()
        # plt.imshow(imslice3d, cmap='gray')
        # plt.show()

        ##check  a single slice of inputs
        # middle_slice_index = self.real_A.shape[2]//2
        # slice_to_display = self.real_A[0, 0, 2, :, :]
        # # Normalize and convert to numpy for plotting, if necessary
        # slice_to_display = slice_to_display.cpu().numpy()
        # ////
        # # Display the chosen slice
        # plt.figure()  # Create a new figure window
        # plt.imshow(slice_to_display, cmap='gray')
        # plt.title("Slice of self.real_A")
        # plt.show()
